...
const cartController = {
  ...
  deleteItem: async function (req, res) {
    try {
      const { id } = req.params;
      const isExist = await CartItem.findOne({ where: { id } });
      if (!isExist) return messages.error(404, "Item not found");

      return await db.sequelize.transaction(async function (t) {
        await CartItem.destroy({ where: { id } },{ transaction: t });
        res.status(200).json({ messages:"Item deleted successfully"})
      });
    } catch (error) {
  ...
  updateCartItem: async function (req, res) {
    try {
      ...
      if (!cartItem) {
        return res.status(404).json(messages.error(404, "Cart item not found"));
      }
  
      cartItem.quantity = quantity;
      await cartItem.save();
  
      return res.status(200).json(messages.success("Cart item updated successfully"));
    } catch (error) {
